...
const API_RESERVATIONS = 'https://api.npoint.io/96777d9f150a95155d45';
const API_EVENTS = 'https://api.npoint.io/3addc354adb3deef04d2';
...
function renderTablesList(filtered) {
  ...
  filtered.forEach(res => {
    ...
    const occasionHtml = res.occasion 
      ...
      : '';
    const wishesHtml = res.specialRequests 
      ? `<div class="guest-meta" style="max-width:220px; font-style:italic;">"${res.specialRequests}"</div>` 
      : '';
    ...
    tr.innerHTML = `
      <td><span class="ref-badge">${res.ref}</span></td>
      <td>
...
function renderEventsList(filtered) {
  ...
  filtered.forEach(rsvp => {
    ...
    tr.innerHTML = `
      <td><span class="ref-badge" style="background:#e5d3c3;">${rsvp.ref}</span></td>
      <td>
  ...
  if (type === 'table') {
    ...
    if (record.dishes && Object.keys(record.dishes).length > 0) {
      ...
      dishesList = `<div class="ticket-preview-divider"></div>
                    <span class="ticket-preview-label">Selected Tastings</span>
    ...
    ticketHtml = `
      ...
      <div class="ticket-preview-body">
        ...
        ${occasionText}
        ${record.specialRequests ? `<div class="ticket-preview-divider"></div><span class="ticket-preview-label">Wishes</span><div style="font-size:0.75rem; font-style:italic;">"${record.specialRequests}"</div>` : ''}
        ${dishesList}
        <div class="ticket-preview-divider"></div>
        ${actionBtn}
      </div>
